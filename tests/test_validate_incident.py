...
from resq_mcp.hce.tools import validate_incident
...
class TestValidateIncident:
    ...
    async def test_validate_incident_confirms(self, caplog: LogCaptureFixture) -> None:
        ...
        # Verify logging (tool normalises incident_id to uppercase for storage)
        assert "INC-CONFIRM-001" in caplog.text
        assert "CONFIRMED" in caplog.text
        assert "Human-Operator" in caplog.text
        assert "Confirmed via visual evidence" in caplog.text
    ...
    async def test_validate_incident_rejects(self, caplog: LogCaptureFixture) -> None:
        ...
        # Verify logging (tool normalises incident_id to uppercase for storage)
        assert "INC-REJECT-002" in caplog.text
        assert "REJECTED" in caplog.text
        assert "Auto-Validator" in caplog.text
        assert "Rejected due to low confidence" in caplog.text
